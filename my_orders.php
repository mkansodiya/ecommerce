...
isLogin();
$user = mysqli_fetch_assoc(logedUserData());
$order_q = getUserOrders($user['user_id']);
?>
...
<section class="section-big-py-space b-g-light">
    <div class="container">
        <div class="row">
            <?php include("inc/account_nav.php"); ?>
            <div class="col-lg-9">
                                ...
                                <div class="card-body">
                                    <div class="user-status table-responsive latest-order-table">
                                        <table class="table table-bordernone">
                                            ...
                                            <tbody>
                                                <?php while ($orders = mysqli_fetch_assoc($order_q)) {
                                                ?>
                                                    <tr>
                                                        <td><a href="http://localhost/ecommerce/order_detail.php?order_id=<?php echo $orders['order_id']; ?>"><?php if (mysqli_num_rows(getOrder($orders['order_id'])) > 1) {
                                                                                                                                                                    echo mysqli_fetch_assoc(getProductByID(mysqli_fetch_assoc(getOrder($orders['order_id']))["product_id"]))['product_name'] . " +" . (mysqli_num_rows(getOrder($orders['order_id']))) - 1 . " other";
                                                                                                                                                                } else {
                                                                                                                                                                    echo mysqli_fetch_assoc(getProductByID(mysqli_fetch_assoc(getOrder($orders['order_id']))["product_id"]))['product_name'];
                                                                                                                                                                } ?></a></td>
                                                        <td class="digits"><?php echo $orders['order_total']; ?></td>
                                                        <td class="font-danger"><?php echo $orders['payment_method']; ?></td>
                                                        <td class="digits"><?php echo $orders['order_status']; ?></td>
                                                    </tr>
                                                <?php }
                                                ?>
                                            </tbody>
